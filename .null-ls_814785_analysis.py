...
def umap(emb_dir, split, aggregate="mean", N=100):
    ...
    plt.scatter(
        umap_emb[: len(clean_data), 0],
        umap_emb[: len(clean_data), 1],
        c="b",
        marker="o",
    )
    plt.scatter(
    ...
    for noisy_sample, clean_sample in zip(
        umap_emb[len(clean_data) :], umap_emb[: len(clean_data)]
    ):
    ...
    noisy_data, clean_data = ot_sampler.sample_plan(
    ...
    umap_emb = reducer.fit_transform(
        torch.concatenate((clean_data, noisy_data)).numpy()
    )
    plt.scatter(
        umap_emb[: len(clean_data), 0],
        umap_emb[: len(clean_data), 1],
