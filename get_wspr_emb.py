...
def process_file(model, audio_path, output_emb_path, output_txt_path, device):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            audio_features = model.embed_audio(
                mel[None, :, :]
            )  # Shape: [n_segments, D_MODEL]
...
def get_wspr_emb(input_dir, output_dir, model_name, device_arg, force_overwrite):
    """
    Main function to orchestrate the dataset processing.
    Accepts individual configuration arguments.

    Args:
        input_dir (str): Path to the root VB+DMD dataset directory.
        output_dir (str): Path to the output directory (e.g., VB+DMD_wspr).
    ...
    # Determine device based on input argument and availability
    device = device_arg  # Start with the requested device
    ...
    try:
        # Use the determined device for loading
        model = whisper.load_model(model_name, device=device, download_root="./.cache")
    except Exception as e:
        print(f"Error loading Whisper model: {e}")
        print(
            "Please ensure 'openai-whisper' and its dependencies (like ffmpeg) are installed correctly."
        )
        return
    ...
    for split in splits:
        for condition in conditions:
            ...
            for wav_path in progress_bar:
                ...
                # Use the force_overwrite argument passed to main
                if (
                    not force_overwrite
                ...
                # Pass the determined device to process_file
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract Whisper embeddings and transcriptions from VB+DMD dataset."
    )
    parser.add_argument(
        "input_dir", type=str, help="Path to the root VB+DMD dataset directory."
    )
    parser.add_argument(
        "output_dir", type=str, help="Path to the output directory (e.g., VB+DMD_wspr)."
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    # --- Call main with individual arguments from the parsed args object ---
    get_wspr_emb(
        input_dir=args.input_dir,
        output_dir=args.output_dir,
        model_name=args.model_name,
        device_arg=args.device,  # Pass the originally requested device arg
